...
SkinnedInfo SkinVert( VSSkinnedIn Input)
{
	...
	Output.Norm = mul(Input.Norm, (float3x3)m);
	Output.Tan = mul(Input.Tan, (float3x3)m);
...
//--------------------------------------------------------------------------------------
PSSkinnedIn VSSkinnedmain(VSSkinnedIn input)	// Движение
{
    PSSkinnedIn output;
...
PSSkinnedIn VSmain(VSSkinnedIn input)	// Без движения
{
    PSSkinnedIn output;
    float4 Pos = float4(input.Pos,1);
	...
	/*Matrix m= m_matConstBoneWorld[input.Bones.x] * input.Weights.x
			+ m_matConstBoneWorld[input.Bones.y] * input.Weights.y
			+ m_matConstBoneWorld[input.Bones.z] * input.Weights.z
			+ m_matConstBoneWorld[input.Bones.w] * input.Weights.w;
	Pos = mul(Pos, m);
	output.Pos = mul(Pos, m_matWorldViewProj);
*/  
    return output;
